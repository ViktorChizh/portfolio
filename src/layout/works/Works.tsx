...
const worksBase: WorksBase[] = [
  {
    ...
    pRus: (
      <p>
        Разработчик 1С с уникальным профилем: 20+ лет в должности главного
        бухгалтера (производство, торговля, ЭДО, цифровая отчётность, налоговый
        и управленческий учёт) + коммерческий опыт современной веб-разработки
        (более 2 лет с JavaScript, TypeScript, React, Next.js, Vue.js).
        <br /> Благодаря опыту главного бухгалтера вижу задачу целиком - от
        бизнес-процесса до конечного отчёта. Инженерная культура, полученная во
        frontend, даёт понимание типизации, архитектуры, чистоты кода и
        тестирования. Это позволяет писать на встроенном языке 1С не «как
        получится», а качественно, структурированно и надёжно.
        <br />
        Тестовые по 1С размещены по ссылке{" "}
        <a
          style={{
            color: "#7572D5",
          ...
          href="https://github.com/ViktorChizh/1C_github"
          target="_blank"
          rel="noreferrer"
        >
          1C_github
        </a>
        . Примеры работ по frontend далее. Полученные сертификаты и рекомендации
        - в Demo.
      </p>
    ),
...
export const Works = () => {
  ...
  return (
    <StyledWorks id="Опыт работы">
      ...
      <Info>
        <p>
          Работы в ООО "ВебДэд", где я работал в качестве fullstack
          программиста, не могу предоставить по причине подписанного соглашения
          о конфиденциальности, но они великолепны! 😉
        </p>
      </Info>
    </StyledWorks>
  );
